...
switch(process.argv[2]) {
  ...
  case 'start':
    rewireModule('react-scripts-ts/scripts/start', loadCustomizer('../config-overrides.dev.js'));
    break;
  // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts-ts/scripts/build.ts', loadCustomizer('../config-overrides.prod.ts'));
    break;
  ...
  case 'test':
    ...
    const customizer = loadCustomizer('../config-overrides.testing.ts');
    proxyquire('react-scripts-ts/scripts/test.js', {
      ...
      '../utils/createJestConfig': (...args) => {
        ...
        const createJestConfig = require('react-scripts-ts/utils/createJestConfig');
